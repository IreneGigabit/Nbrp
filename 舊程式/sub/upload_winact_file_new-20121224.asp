...
'(3) gbrDir(區所檔案的實體路徑ex.\\sin09\NPE)    (4)gDir(國外所檔案的實體路徑 ex.\\sin20\FPE_File)
...
select case lcase(session("type"))
	...
	case "custdb_file","db_file","custresp_file"	'2012/5/2增加，custdb_file=對催帳客函 db_file=請款單 custresp_file=客戶對催回應文件
	   	folder_name = session("folder_name")
		prefix_name = session("prefix_name")
		draw_file = session("draw_file")
		old_file = right(draw_file,len(draw_file)-InstrRev(draw_file,"\"))
	   'file_path="\FEXP\FPE_file\" & folder_name
	   if lcase(session("type"))="custdb_file" or lcase(session("type"))="custresp_file" then
    ...
    if session("type")="custdb_file" or session("type")="db_file" or session("type")="custresp_file" then
	  ...
	  <%else
		...
		<%Else
			...
			'Response.Write session("filename_flag") &"<BR>"
			'Response.end
