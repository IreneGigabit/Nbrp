...
select case trim(request("type"))
  ...
  case "doc"
      session("type")="doc"
	  session("folder_name")=trim(Request("folder_name"))	'檔案目錄所在,會在cust_area之下建立
	  session("prefix_name")=trim(Request("prefix_name"))	'檔案的前置名稱:例如: filename=abc.jpg, if prefix="123" , then filename=123_abc.jpg,用於區隔同目錄底下,不同的檔案名稱
      session("draw_file")=trim(Request("draw_file"))	'原檔案路徑(存在於server上的D:\data\document)
      session("form_name")=trim(Request("form_name"))
      session("size_name")=trim(Request("size_name"))
      session("file_name")=trim(Request("file_name"))
      session("btnname")=trim(Request("btnname"))
      cont="檔案上傳"
  case "Ext_photo"
      session("type")="photo"
      session("seq")=Request("seq")
      session("cust_area")=left(Request.QueryString("cust_area"),1)&"TE"
      session("draw_file")=Request("draw_file")
      '93/03/08_jessica修改
      session("form_name")=Request("form_name")
      '93/03/08_end
