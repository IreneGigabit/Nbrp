...
function getseqEU(pkind)
	...
	for k=1 to 33
		...
		elseif asc(pkind)=83 then   '到S之後跳過T
			pkind = "U"
			if chkseqEU(pkind) = false then
				getseqEU = pkind
				exit function
...
'      若無,則不用修改案件主檔
'      若有,則需找到最後一個有案件狀態的 
function update_case_stat_dmp(pconn,pseq,pseq1,pstep_grade,prs_type,prs_class,prs_code,pact_code,pcase_stat,prs_sqlno)
	Dim tisql
	Dim rst
	Dim tcase_stat,trs_type,trs_class,trs_code,tnow_grade
	Dim fcase_stat,frs_type,frs_class,frs_code,fstep_grade
	...
	if pcase_stat <> "" then
		...
		'表示傳入案性沒有對應案件狀態
		'檢查now_grade是否等於pstep_grade,如果是則找到最後一筆有案件狀態
		tisql = "select now_grade from dmp "
		tisql = tisql & " where seq = " & pseq
		tisql = tisql & "   and seq1 = '" & pseq1 & "'"
		'Response.write tisql & "<Br>"
		'Response.end
	...
	if fstep_grade <> empty then
		...
		if session("scode")="m983" then Response.Write "修改案件狀態:" & tisql & "<br>"
		'Response.end
		pconn.execute tisql 
		if err.number<>0 then
			update_case_stat = true
...
'入檔後檢查主檔有無立案日、申請日、申請號
function dmp_check_data(pconn,pseq,pseq1,pchktype)
	dmp_check_data = "N"
	fchktypenm = ""
	set frs = Server.CreateObject("ADODB.Recordset")
	fsql = "select * from dmp where seq='"& pseq &"' and seq1='"& pseq1 &"'"
	frs.open fsql,pconn,1,1
	if not frs.eof then
		select case pchktype
			case "A"
				if trim(frs("in_date"))<>empty then
				else
					fchktypenm = fchktypenm & "立案日、"
					dmp_check_data = "Y"
				end if
				if trim(frs("apply_date"))<>empty then
				else
					fchktypenm = fchktypenm & "申請日、"
					dmp_check_data = "Y"
				end if
				if trim(frs("apply_no"))<>empty then
				else
					fchktypenm = fchktypenm & "申請號、"
					dmp_check_data = "Y"
				end if
				if trim(frs("case1"))<>empty then
				else
					fchktypenm = fchktypenm & "專利種類、"
					dmp_check_data = "Y"
				end if
			case "B"
				if trim(frs("in_date"))<>empty then
		...
		if fchktypenm<>empty then
			fchktypenm = left(fchktypenm,len(fchktypenm)-1)
		end if
	end if
	frs.close
	
	'dmp_check_data = "Y" 'test
	if dmp_check_data = "Y" then
		tmailto="m983@saint-island.com.tw"
		Sender = session("se_scode") '寄件者
		fseq = session("se_branch") & "P" & pseq
		if pseq1<>"_" then fseq = fseq & "-" & pseq1
		
		tsubject = ""
		tsubject = tsubject & fseq &"－主檔"& fchktypenm &"無資料"& prgid &"－"& Request.ServerVariables("SERVER_NAME")
		
		body_temp = "select * From dmp_log a,dmp b where a.ud_date>='2011/4/18 00:00:00'<br>"
		body_temp = body_temp & " and a.seq=b.seq and a.seq1=b.seq1 and ((isnull(a.apply_no,'')<>'' and isnull(b.apply_no,'')='') or (a.apply_date is not null and b.apply_date is null) or b.in_date is null or b.case1='' or b.case1 is null)<br>"
		body_temp = body_temp & "select seq,seq1,case1,in_date,apply_no,apply_date,issue_no,issue_date,capply_no,pay_date,pay_times,term1,term2,* From dmp where seq="& pseq &" and seq1='"& pseq1 &"'<br>"
		body_temp = body_temp & "select seq,seq1,ud_date,prgid,case1,in_date,apply_no,apply_date,issue_no,issue_date,capply_no,pay_date,pay_times,term1,term2,* From dmp_log where seq="& pseq &" and seq1='"& pseq1 &"' order by sqlno<br>"
		body_temp = body_temp & "select * From step_dmp where seq="& pseq &" and seq1='"& pseq1 &"'<br>"
		body_temp = body_temp & "--update dmp set case1='IG1' where seq="& pseq &" and seq1='"& pseq1 &"'<br>"
		body_temp = body_temp & "--update dmp set in_date='2007/8/2' where seq="& pseq &" and seq1='"& pseq1 &"'<br>"
		body_temp = body_temp & "--update dmp set apply_date='2007/10/24' where seq="& pseq &" and seq1='"& pseq1 &"'<br>"
		body_temp = body_temp & "--update dmp set apply_no='100208879' where seq="& pseq &" and seq1='"& pseq1 &"'<br>"
		body_temp = body_temp & "--update dmp set issue_date='2007/6/11' where seq="& pseq &" and seq1='"& pseq1 &"'<br>"
		body_temp = body_temp & "--update dmp set issue_no='282382' where seq="& pseq &" and seq1='"& pseq1 &"'<br>"
		body_temp = body_temp & "--update dmp set pay_date='2014/8/10' where seq="& pseq &" and seq1='"& pseq1 &"'<br>"
		body_temp = body_temp & "--update dmp set pay_times='03' where seq="& pseq &" and seq1='"& pseq1 &"'<br>"
		body_temp = body_temp & "--update dmp set capply_no='M409425' where seq="& pseq &" and seq1='"& pseq1 &"'<br>"
		body_temp = body_temp & "--update dmp set term1='2011/8/11' where seq="& pseq &" and seq1='"& pseq1 &"'<br>"
		body_temp = body_temp & "--update dmp set term2='2021/2/22' where seq="& pseq &" and seq1='"& pseq1 &"'<br>"
	
		'set objMail=createobject("cdonts.newmail")
		'objmail.mailformat = cdomailformatmime
		'objmail.from= Sender&"@saint-island.com.tw"
		'objmail.to = tmailto
		''objmail.cc = strtolist
		'objmail.mailformat=cdomailformatMIME
		'objmail.BodyFormat=cdoBodyFormatHTML
		'objmail.subject = tsubject
...
'出專管制案件
function getPE_Ignore(pConn,pBranch,pTableName)
	set rst = server.CreateObject("Adodb.recordset")
	if pBranch="" then
		fsql = "select * from cust_code where Code_type='PE_Ignore' and cust_code='"&session("se_Branch")&"'"
	else
		fsql = "select * from cust_code where Code_type='PE_Ignore' and cust_code='"&pBranch&"'"
	end if
	rst.open fsql,pConn,1,1
	rtnStr=""
	if not rst.eof then
		arrRemark=split(rst("remark")&"",";")
		'ReDim ignore( UBound(arrRemark) , 2 )
		For i = LBound(arrRemark) to UBound(arrRemark)-1
			'ignore(0,1)=Mid(arrRemark(i),3,Len(Match.value)-3)
			if rtnStr<>"" then
				rtnStr=rtnStr &" or"
			end if
			if pTableName<>"" then
				if InStr(arrRemark(i),"-")>0 then
					rtnStr=rtnStr &" (isnull("& pTableName&".seq,'')='"&Mid(arrRemark(i),1,InStr(arrRemark(i),"-")-1) & "' "
					rtnStr=rtnStr &" and "& pTableName&".seq1='"&Mid(arrRemark(i),InStr(arrRemark(i),"-")+1,len(arrRemark(i))) & "') "
				else
					rtnStr=rtnStr &" (isnull("& pTableName&".seq,'')='"&arrRemark(i) & "' "
					rtnStr=rtnStr &" and "& pTableName&".seq1='') "
				end if
			else
				if InStr(arrRemark(i),"-")>0 then
					rtnStr=rtnStr &" (isnull(seq,'')='"&Mid(arrRemark(i),1,InStr(arrRemark(i),"-")-1) & "' "
					rtnStr=rtnStr &" and seq1='"&Mid(arrRemark(i),InStr(arrRemark(i),"-")+1,len(arrRemark(i))) & "') "
				else
					rtnStr=rtnStr &" (isnull(seq,'')='"&arrRemark(i) & "' "
					rtnStr=rtnStr &" and seq1='') "
				end if
			end if
		next
	end if
	rst.close
	if rtnStr="" then rtnStr=" 1=0 "
	getPE_Ignore=rtnStr
end function
'出專管制案件
function getPE_IgnoreList(pConn,pBranch)
	set rst = server.CreateObject("Adodb.recordset")
	if pBranch="" then
		fsql = "select * from cust_code where Code_type='PE_Ignore' and cust_code='"&session("se_Branch")&"'"
		rBranch=session("se_Branch")
	else
		fsql = "select * from cust_code where Code_type='PE_Ignore' and cust_code='"&pBranch&"'"
		rBranch=pBranch
	end if
	...
	if not rst.eof then
		...
		For i = LBound(arrRemark) to UBound(arrRemark)-1
			'ignore(0,1)=Mid(arrRemark(i),3,Len(Match.value)-3)
			rtnStr=rtnStr & rBranch & "-" & arrRemark(i) & ";"
		next
	end if
	rst.close
	getPE_IgnoreList=rtnStr
end function
%>
