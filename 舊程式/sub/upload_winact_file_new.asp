...
'Response.Write "session(""type"")="&session("type") & "<BR>"
'Response.End
...
'response.write "session(""draw_file"")="&session("draw_file") &"<BR>"
'response.write "strreverse(bb)="&strreverse(bb) &"<BR>"
'response.write "instr(strreverse(bb),""."")="&instr(strreverse(bb),".") &"<BR>"
'File_name_temp=left(bb,len(bb)-instr(strreverse(bb),".")) & "_" & datepart("yyyy",date()) & string(2-len(datepart("m",date())),"0") & datepart("m",date()) & string(2-len(datepart("d",date())),"0") & datepart("d",date()) & string(2-len(hour(time)),"0") & hour(time) & string(2-len(minute(time)),"0") & minute(time) & string(2-len(second(time)),"0") &second(time) & right(bb,instr(strreverse(bb),"."))
'response.write "File_name_temp="&File_name_temp &"<BR>"
'response.write "file_path&""/""&dd="&file_path&"/"&dd &"<BR>"
'Response.End 
'end if
if bb = old_file1 then
	...
	if session("type")="custdb_file" or session("type")="db_file" or session("type")="custresp_file" then
		...
		'Response.Write "Server.MapPath(chkfilename)=" & Server.MapPath(chkfilename) &"<BR>"
		if chkFileExist(chkfilename) = 1 then%>
			...
			<%Response.end
		elseif chkFileExist_virtual(chkfilename) = 1 then%>
			<SCRIPT LANGUAGE=vbs>
			msgbox "該檔案已經存在!!" & chr(10) & chr(10) & "請將該檔案更名，並重新上傳!!" 
			window.close
			</SCRIPT>
