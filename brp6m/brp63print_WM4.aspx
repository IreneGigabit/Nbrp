...
<script runat="server">
	...
	protected void WordOut() {
		...
		_tplFile.Add("apply", Server.MapPath("~/ReportTemplate/申請書/11專利更正申請書WM4.docx"));
		_tplFile.Add("base", Server.MapPath("~/ReportTemplate/申請書/00基本資料表.docx"));
		ipoRpt.CloneFromFile(_tplFile, true);

		string docFileName = ipoRpt.Seq + "-[專利更正申請書].docx";
		...
		if (dmp.Rows.Count > 0) {
			...
			if (dmp.Rows[0]["change_no"].ToString() != "") {
				apply_no = dmp.Rows[0]["change_no"].ToString();
			} else {
				apply_no = dmp.Rows[0]["apply_no"].ToString();
			}
			//若是舉發案則申請號填在舉發案號
			if (dmp.Rows[0]["CASE1"].ToString() != "OG") {
				//申請案號
				ipoRpt.ReplaceBookmark("apply_no", apply_no);
				ipoRpt.ReplaceBookmark("og_apply_no", "");
			} else {
				//舉發案號
				ipoRpt.ReplaceBookmark("apply_no", "");
				ipoRpt.ReplaceBookmark("og_apply_no", apply_no);
			}
			//專利證書號數
