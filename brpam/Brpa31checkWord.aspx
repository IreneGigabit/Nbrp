...
<script runat="server">
	...
	private void Page_Load(System.Object sender, System.EventArgs e) {
		...
		try {
			...
			MatchCollection Matches = Regex.Matches(title_line, @"【(?<tag>.*)】(?<value>.*)", RegexOptions.IgnoreCase);
			foreach (Match match in Matches) {
				Response.Write("..." + match.Groups["tag"].Value + "...<BR>");
			...
			List<Content> attachList = GetAttachList();
			foreach(var item in attachList) {
				Response.Write(item.Tag+"<BR>");
	...
	protected List<Content> GetAttachList() {
		List<Content> attach_list = new List<Content>();
		...
		if (wordApp.Selection.Find.Execute(ref oMissing,
			...
			while (++i < 100) {//防止無限迴圈
				...
				if (strTemp.IndexOf("【檔案具結】") > -1 || strTemp == "【本申請書所檢送之PDF檔或影像檔與原本或正本相同】" || strTemp == "【本申請書所填寫之資料係為真實】") {
					...
					MatchCollection Matches = Regex.Matches(strTemp, @"【(?<tag>.*)】(?<value>.*)", RegexOptions.IgnoreCase);
					foreach (Match match in Matches) {
